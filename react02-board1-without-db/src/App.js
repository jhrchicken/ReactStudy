  ...
  if (mode === 'list') {
    ...
    navComp  = <NavEdit
      ...
      articleComp = <ArticleEdit selectRow = {selectRow}
        editAction = {(t, w, c) => {
          /* 사용자가 입력한 값을 통해 새로운 객체를 생성한다. 단, 일련번호와 작성일은 기존의 값을 그대로 사용한다. */
          ...
          setBoardData(copyBoardData);
          // 수정된 내용 확인을 위해 '열람' 페이지로 전환한다.
          setMode('view');
        }}></ArticleEdit>
  }
  else {
    // mode의 값이 없는 경우 '준비중'을 화면에 출력한다.
